 ...
 */
struct PagerankConverge: public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &vertex, graphchi_context &gcontext) {
        ...
        if (gcontext.iteration == 0) {
			for( int i=0; i<vertex.num_outedges(); i++) {
				graphchi_edge<EdgeDataType>* edge=vertex.outedge(i);
				pthread_mutex_init( &(edge->data_ptr->mutex), NULL );
				edge->data_ptr->value = 1.0/vertex.num_outedges();
			}
			...
			if(enable_mutex)	
                ...
                float val = vertex.inedge(i)->data_ptr->value;
			if(enable_mutex)
            ...
            if (vertex.num_outedges() > 0) {
                ...
                for(int i=0; i < vertex.num_outedges(); i++) {
                    graphchi_edge<EdgeDataType> * edge = vertex.outedge(i);
				if(enable_mutex)
					pthread_mutex_lock( &edge->data_ptr->mutex );
					edge->data_ptr->value = pagerankcont;
				if(enable_mutex)
					pthread_mutex_unlock( &edge->data_ptr->mutex );
                }
            }
...
int main(int argc, const char ** argv) {
    ...
    int nshards             = (int) convert_if_notexists<float, tmp>(filename, get_option_string("nshards", "auto"));
